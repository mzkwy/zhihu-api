...
exports.followers = listFollows(urls.followers)
exports.followees = listFollows(urls.followees)
...
function getProfileCard(uname) {
    return request(urls.user.profilecard(uname))
        .then(_parseProfileCard)
}

function listFollows(followUrl) {
        ...
        return request.xsrf()
            .then(_xsrf => ({
                ...
                params: JSON.stringify({
                    order_by: 'created',
                    hash_id: profile.hash,
                    offset
                }),
                _xsrf
            }))
            .then(data => request(followUrl, data))
            .then(data => {
