...
function parseUserColumns(html) {
    var $ = cheerio.load(html)
    var eles = $('#zh-profile-posts .column')

    return util.map(eles, ele => _parseUserColumn($, $(ele)))
}

function _parseUserColumn($, ele) {
    var linkEle = $('.header > a', ele)
    var link = getAttr(linkEle, 'href')
    var avatarEle = $('.Avatar', linkEle)
    var followEle = $('.header .followers-link', ele)
    var postEle = $('.posts .post', ele)
    var footerEle = $('.footer > a', ele)
